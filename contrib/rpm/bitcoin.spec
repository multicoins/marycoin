...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33332
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33333
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 38332
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 38333
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
        %{_sbindir}/semanage port -d -p tcp 33332
        %{_sbindir}/semanage port -d -p tcp 33333
        %{_sbindir}/semanage port -d -p tcp 38332
        %{_sbindir}/semanage port -d -p tcp 38333
	for selinuxvariant in %{selinux_variants}; do
